...
export function ResponseGrid({ data, searchQuery, onGridReady, onMatchesFound }: ResponseGridProps) {
	...
	const handleCellClicked = useCallback((params: any) => {
		...
		const text = typeof val === 'object' ? JSON.stringify(val) : String(val)
		lastClickedValueRef.current = text

		// Auto-copy on click (main's behavior integrated with fix's ref)
	...
	// Listen for Ctrl+C / Cmd+C at document level to copy the clicked cell value (main's behavior)
	useEffect(() => {
